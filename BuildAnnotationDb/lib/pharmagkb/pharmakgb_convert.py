...
geneTestCollection = annoDb['geneTest']
pharmgkb_dir = "/Users/tizatt/Desktop/pharmvar.pharmgkb"
drug_label_dir = "drugLabels"
search_pattern = "*.tsv"

coord_files = {"clinical_ann.tsv": "Location", "study_parameters": "", "variant_drug_ann.tsv": "Variant",
              "var_fa_ann.tsv": "Variant", "var_pheno_ann": "Variant", "automated_annotations": "Variation Name",
              "clinicalVariants": "variant"}
...
def load_to_coord(coord_file, name):
    counter = 0
    bulk = testCollection.initialize_unordered_bulk_op()
    ...
    for record in coord_file:
        ...
        if record[search_id].startswith("rs"):
            ...
            bulk.find({'RS': rs_id}).update(
                {'$set': {"pharmgkb." + name : record}})
            counter += 1

            if counter % 500 == 0:
                bulk.execute()
                bulk = testCollection.initialize_ordered_bulk_op()
...
def load_drug_labels(drug_labels):
    ...
    label_file = ""
    counter = 0
    bulk = geneTestCollection.initialize_unordered_bulk_op()

    for filepath in drug_labels:
        if os.path.basename(filepath) == "drugLabels.byGene":
    ...
    for drug_gene_label in label_by_gene_file:
        ...
        bulk.find({"gene": gene_name}).update(
            {'$set': {"pharmgkb_drug": drugs}})
        counter += 1

        if counter % 500 == 0:
            bulk.execute()
            bulk = geneTestCollection.initialize_ordered_bulk_op()

    if counter % 500 != 0:
        bulk.execute()


def get_files(type, dir):
    files = []
    for d_name, sd_name, f_list in os.walk(dir):
        for file_name in f_list:
            if fnmatch.fnmatch(file_name, search_pattern):
                if type == "coord" and file_name in coord_files:
                    # Match search string
                    files.append(os.path.join(d_name, file_name))
                elif type == "gene" and file_name in gene_files:
                    files.append(os.path.join(d_name, file_name))
    return files


coord_files = get_files("coord", pharmgkb_dir)
for file in coord_files:
    filename = os.path.basename(file)
    subdoc_name = filename.replace(".tsv","")
    file_dict = file_management.load_file(file)
    load_to_coord(file_dict, subdoc_name)

drug_labels = get_files("gene", drug_label_dir)
