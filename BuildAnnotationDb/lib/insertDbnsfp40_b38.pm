...
#      my $md5=addToCollections(@_);
      ...
      my @FILES=`ls -1 $filePath`;
      #my @FILES=`ls -1 db/dbnsfp/dbnsfp4.1links/dbNSFP*chr*`;
	  ...
	  open(FIELDS, '<', "/labs/ngd-data/prodCentralDB/BuildAnnotationDb/db/dbnsfp/dbnsfp4.1/dbnsfp4.1.fields.types") or die $!;
	  while (<FIELDS>) {
	    ($field, $type, $filter) = split;
	    push( @{$dbnsfpFieldTypes{$field}}, $type);
	    push @dbnsfpFields, $field
	  }
	  ...
	  foreach $file (@FILES) {
		  ...
		  LOOP: while(<FILE>) {
			  ...
			  %hash=();
			  %hashAnnotate=();
			  $hash{'coord'}=join(':',"chr" . $lineArr[0],$lineArr[1],$lineArr[2],$lineArr[3]);
			  if (exists($done->{$hash{'coord'}})){next LOOP}               

			  for my $i (0 .. @lineArr-1){
			    my $fieldName = $dbnsfpFields[$i];
			    my $value = $lineArr[$i];
			    my $type = "";
			    my $emptyvalue = "";
			    foreach(@{$dbnsfpFieldTypes{$fieldName}}){
			        $type = $_;
			    }

			    if ( $value eq '.' || $value eq "./." || $value eq "././." || $value eq ".;.;." ||
			         $value eq ".;." || $value eq "0" || $value eq ".;.;" || $value eq "0.0" ||
			         $value eq "0.00" || $value eq "0;0;0" || $value eq ".;.;0.0"){
			        ...
			        my $original_v = $values[0];
			        my $include_all_values = "no";
			        foreach $v (@values){
			            if ( $v ne $original_v){
                            		$include_all_values = "yes";
                            		last;
			            }
			        }
                    		if ( $include_all_values eq "yes"){
                        		$hash{$fieldName} = $value
                    		}
                    		else{
                        		$hash{$fieldName} = $values[0]
                    		}
			    }
			...
			#	 }
			 # }			  
			  $done->{$hash{'coord'}}=1;
			  $dbnsfp->update_one({'coord'=>$hash{'coord'}},{'$set'=>\%hash},{'upsert'=>1,'safe'=>1});
     		}
		close (FILE);
	 }
...
#      chomp($md5sum);
      ...
      $collections->update_one({'md5sum'=>$md5sum},{'date'=>$date,'type'=>$type,'collection'=>$collection},{'upsert'=>1,'safe'=>1});
      return;
}
